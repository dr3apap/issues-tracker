...
const IssuesInfo = ({ count, state }: DashInfoProps) => {
    ...
    return <ul className="py-8">
        <li className="flex flex-col gap-y-2 px-4 py-2 text-[var(--text-color)] text-fluid--1 text-center rounded-md bg-[var(--surface)]">
            ...
            <Link to={`/user/dashboard/${state.toLowerCase().trim()}`} className="underline decoration-4 decoration-green-400">Preview</Link>
        </li>
    </ul >
}
...
export default function IssueDashInfo() {
    ...
    return (
        <div className="pageWrapper">
            {issueStatus.map((status) => <IssuesInfo key={status} state={status} count={status == "Open" ? open.length : status == "In Progress" ? inprogress.length : closed.length} />)}
            <Outlet />
        </div>
    );
}
